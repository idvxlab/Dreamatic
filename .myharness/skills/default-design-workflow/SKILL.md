...
- `video_generate`: optional text-to-video or image-to-video production when
  the user explicitly requests a time-based deliverable.
...
11. Spawn `design-designer` with `default-design-workflow`,
    ...
    and the required artifact contract. When `resolvedScope.optional_video` is
    enabled, include its video plan and require the real video and metadata
    paths. Verify PNG artifacts, any requested video, `artifacts/00-gallery.html`,
    lint result, and `design_done`.
...
The default final deliverable shape is a curated PNG image set plus one polished
`artifacts/00-gallery.html`. Video is not generated by default. When the user
explicitly requests a time-based deliverable, add the requested video as a
supplementary artifact without reducing the required PNG set. The harness
produces concept/effect media and presentation material, not CAD, BIM,
engineering drawings, manufacturing files, or construction documents.
...
```json
{
  ...
  "optional_video": {
    ...
    "source_anchor_intent": "static consistency anchor used to derive a dedicated video first frame"
  },
...
the canonical `design_done` bus message exist. When
`resolvedScope.optional_video.enabled` is true, the requested video file and
its dedicated first-frame image and metadata must also exist and be referenced
